...
# Load expressway
```{r}
expressway<- readOGR(dsn = 'RoadNetwork', layer = 'roads_expressway_small')
```
# Attempt to reduce size of expressway
```{r}
new <- gSimplify(expressway, tol = 100000,  topologyPreserve= TRUE)
object.size(expressway)
object.size(new)
plot(expressway)
plot(new)
```
